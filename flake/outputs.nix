...
, ...
  ...
  in {
    ...
    packages.default = self.packages.GaP;

    # Executed by `nix run <URL of this flake>#GaP -- <args?>`
    # TODO: switch to clang environment
    packages.GaP = pkgs.stdenv.mkDerivation {
      pname = "GaP";
      version = "0.0.0";
      src = "${self}/src";
      nativeBuildInputs = [];
    ...
    apps.default = self.apps.GaP;

    # Executed by `nix run <URL of this flake>#GaP`
    apps.GaP = let
      g4-data = nain4.deps.g4-data-package;
      GaP-app-package = pkgs.writeShellScriptBin "GaP" ''
        export PATH=${pkgs.lib.makeBinPath [ self.packages.GaP ]}:$PATH
        ...
        exec GaP --macro-path ${self}/macs "$@"
      '';
    in { type = "app"; program = "${GaP-app-package}/bin/GaP"; };
    ...
    packages.singularity = pkgs.singularity-tools.buildImage {
      name = "GaP";
      contents = [ self.apps.GaP.program ];
      runScript = "${self.apps.GaP.program} $@";
      diskSize = 10240;
      memSize = 5120;
    };
